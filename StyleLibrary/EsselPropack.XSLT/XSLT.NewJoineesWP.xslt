  ...
  <xsl:template match="/" xmlns:x="http://www.w3.org/2001/XMLSchema">
    <span class="ep-webpart">
      <span class="ep-webpart-header">
        <span id="img-newjoinees" class="ep-webpart-img"/>
        <!--img-directorsmessage-->
        <span class="ep-webpart-title">New Joinees</span>
      </span>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr>
            <td style="text-align:left; vertical-align:top; padding-left:5px">
              <xsl:variable name="ArchiveViewLink" select="concat($HttpVDir,'/Lists/NewJoinees/ArchiveView.aspx?Source=',$PageUrl,'&amp;InitialTabID=Ribbon.Read')" />
            ...
            <xsl:choose>
              <xsl:when test="$ListRight_AddListItems">
                <td style="text-align:right; vertical-align:top; padding-right:5px">
                  <span class="ep-webpart-footer">
                    <xsl:variable name="NewPageLink" select="concat($HttpVDir,'/Lists/NewJoinees/NewForm.aspx?Source=',$PageUrl,'&amp;InitialTabID=Ribbon.Read')" />
                    <a href="{$NewPageLink}">
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr  class="ep-webpart-record">
        <td style="width: 30%; vertical-align:top;">
          <img class="ThumbnailStyle" src='{$ThumbnailURL}' alt="{@EmployeeName}"> </img>
        </td>
        <td style="vertical-align:middle; padding-left:5px;">
          <xsl:variable name="Id">
            <xsl:value-of select="@ID"/>
          </xsl:variable>
          <xsl:variable name="DetailPageLink" select="concat($HttpVDir,'/Lists/NewJoinees/ReadMore.aspx?ID=',$Id,'&amp;Source=',$PageUrl,'&amp;InitialTabID=Ribbon.Read')" />
      ...
      <!--<tr>
        <td style="text-align:right" colspan="2">
          ...
          <xsl:variable name="DetailPageLink" select="concat('/Lists/NewJoinee/ReadMore.aspx?ID=',$Id,'&amp;Source=',$PageUrl)" />
          <a href="{$DetailPageLink}">
            Read More <span class="ep-readmorearrow"></span>
          </a>
        </td>
      </tr>-->
      <tr>
