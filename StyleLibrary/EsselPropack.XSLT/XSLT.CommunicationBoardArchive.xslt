...
-->
    ...
    <xsl:choose>
      <xsl:when test="$ListRight_AddListItems">
        <span class="ep-webpart-footer" style="text-align:right; vertical-align:top; padding-right:5px">
          <xsl:variable name="NewPageLink" select="concat($HttpVDir,'/Lists/CommunicationBoard/NewForm.aspx?Source=',$PageUrl,'&amp;InitialTabID=Ribbon.Read')" />
          <a href="{$NewPageLink}">
  ...
  <xsl:template name="dvt_1.rowview">
    ...
    <xsl:variable name="itemurl" select="concat($HttpVDir,'/Lists/CommunicationBoard/ReadMore.aspx?ID=',$Id,'&amp;Source=',ddwrt:UrlEncode($PageUrl),'&amp;InitialTabID=Ribbon.Read')"></xsl:variable>
    ...
    <table class="tbl-borderbottom" width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr  class="ep-archive-record">
        <td style="width: 10%; vertical-align:top; padding-top:8px">
          <img class="ThumbnailStyle" src='{$Thumbnail}' alt="Communication Board Image"> </img>
        </td>
        <td style="vertical-align:top; padding-left:5px;">
          ...
          <span class="ep-announcement-shortdescription">
            ...
            <br/>
            <xsl:value-of select="substring-before(substring-after(@Country, '&gt;'), '&lt;')" disable-output-escaping="yes" /> | <xsl:value-of select="substring-before(substring-after(@Unit, '&gt;'), '&lt;')" disable-output-escaping="yes" /> : <xsl:value-of select="@Price" disable-output-escaping="yes" />
            <br />
        ...
        <td class="td-leftborder" style="width: 12%;vertical-align:middle">
          ...
          <xsl:variable name="DetailPageLink" select="$itemurl" />
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
