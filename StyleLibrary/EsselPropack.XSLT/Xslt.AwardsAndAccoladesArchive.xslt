  ...
  <xsl:template match="/" xmlns:x="http://www.w3.org/2001/XMLSchema" xmlns:d="http://schemas.microsoft.com/sharepoint/dsp"
    ...
    <span class="ep-webpart">
      <!--<span class="ep-webpart-header">
        <span id="img-knowledgesharing" class="ep-webpart-img"/>
        <span class="ep-webpart-title">Announcements</span>
      </span>-->
      <span class="ep-webpart-body">
        <xsl:call-template name="dvt_1.topview" />
...
-->
  <xsl:template name="dvt_1.topview">
    <!--Check For New Item  Permission...-->
    <xsl:choose>
      <xsl:when test="$ListRight_AddListItems">
          <span class="ep-webpart-footer" style="text-align:right; vertical-align:top; padding-right:5px">
            <xsl:variable name="NewPageLink" select="concat($HttpVDir,'/Lists/AwardsAndAccolades/NewForm.aspx?Source=',$PageUrl,'&amp;InitialTabID=Ribbon.Read')" />
            <a href="{$NewPageLink}">
              <img src="/EsselImages/Icons/AddNew.jpg" height="14px" width="14px" vertical-align="middle"></img>Add New
            </a>
          </span>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="dvt_1.rowview">
    ...
    <xsl:variable name="AwardImageLink">
      <xsl:choose>
    ...
    <table class="tbl-borderbottom" width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr  class="ep-archive-record">
        
        <td style="width: 10%; vertical-align:top; padding-top:8px">
          <img class="ThumbnailStyle" src='{$AwardImageLink}' alt="Award Image"> </img>
        </td>
        
        <td style="vertical-align:top;padding-left:5px; ">          
          <span class="ep-announcement-title" style="display:block;padding-left:0px;">
          ...
          <span>            
            ...
            <xsl:choose>     
                <xsl:when test="@Department!=''">
                  <tr>
                    <td>
                      <span style="color:rgb(0, 114, 198);"> Department  </span> : 
                    </td>
                    <td>
                      [<i><xsl:value-of select="substring-before(substring-after(@Department, '&gt;'), '&lt;')" disable-output-escaping="yes"/>  </i>]
                    </td>  
                  </tr>
              </xsl:when>
              </xsl:choose> 
           ...
           <xsl:choose>     
                <xsl:when test="@Region!=''">
                  <tr>
                    <td>
                      <span style="color:rgb(0, 114, 198);"> Region  </span> : 
                    </td>
                    <td>
                      [<i><xsl:value-of select="substring-before(substring-after(@Region, '&gt;'), '&lt;')" disable-output-escaping="yes"/>  </i>]
                    </td>  
                  </tr>
              </xsl:when>
              </xsl:choose> 
            <xsl:choose>     
                <xsl:when test="@Unit!=''">
                  <tr>
                    <td>
                      <span style="color:rgb(0, 114, 198);"> Unit  </span> : 
                    </td>
                    <td>
                      [<i><xsl:value-of select="substring-before(substring-after(@Unit, '&gt;'), '&lt;')" disable-output-escaping="yes"/>  </i>]
                    </td>  
                  </tr>
              </xsl:when>
              </xsl:choose> 
            <xsl:choose>     
                <xsl:when test="@Country!=''">
                  <tr>
                    <td>
                      <span style="color:rgb(0, 114, 198);"> Country  </span> : 
                    </td>
                    <td>
                      [<i><xsl:value-of select="substring-before(substring-after(@Country, '&gt;'), '&lt;')" disable-output-escaping="yes"/>  </i>]
                    </td>  
                  </tr>
              </xsl:when>
              </xsl:choose>  
        ...
        <td class="td-leftborder" style="width: 12%;vertical-align:middle">
          ...
          <xsl:variable name="DetailPageLink" select="concat($HttpVDir,'/Lists/AwardsAndAccolades/ReadMore.aspx?ID=',$Id,'&amp;Source=',ddwrt:UrlEncode($PageUrl),'&amp;InitialTabID=Ribbon.Read')" />
          <table border="0" cellpadding="0" cellspacing="0" width="100%">            
            <tr>
            ...
            <a style="font-size:20px;">
              <xsl:choose>
                ...
                <xsl:otherwise>
                  ...
                  <img src="/_layouts/{$LCID}/images/prev.gif" border="0" alt="{$Rows/@idPrevious}" />Prev
                </xsl:otherwise>
              </xsl:choose>
            </a>
          </xsl:if>
